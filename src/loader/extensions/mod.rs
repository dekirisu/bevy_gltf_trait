...
pub struct GltfExtensionHandlers<G:GltfTrait>(pub Arc<RwLock<Vec<Box<dyn GltfExtensionHandler<G>>>>>);
...
pub trait GltfExtensionHandler<G:GltfTrait>: Send + Sync {
    /// Required for dyn cloning
    fn dyn_clone(&self) -> Box<dyn GltfExtensionHandler<G>>;
...
impl <G:GltfTrait> Clone for Box<dyn GltfExtensionHandler<G>> {
    fn clone(&self) -> Self {
        self.dyn_clone()
    }
}
